...
export const knowledgeSearchParamsSchema = Type.Object({
  ...
  threshold: Type.Optional(
    Type.Number({ description: "Similarity threshold (0-1)", default: 0.8 }),
  ),
  limit: Type.Optional(
...
export const knowledgeWriteParamsSchema = Type.Object({
  ...
  metadata: Type.Optional(Type.Record(Type.String(), Type.Unknown())),
  sourceRunId: Type.Optional(
    Type.String({ description: "Originating run identifier" }),
  ),
});
...
async function executeKnowledgeSearch(
  ...
  const results = await ctx.db
    ...
    .where(
      and(gte(similarity, threshold), eq(knowledgeSnippets.orgId, ctx.orgId)),
    )
...
async function executeKnowledgeWrite(
  params: KnowledgeWriteParams,
  ctx: ExecutionContext,
) {
