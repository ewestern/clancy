...
import { CapabilityMeta, ProviderRuntime } from "../providers/types.js";
import { eq, gt, and } from "drizzle-orm";


function hasPermissions(capabilityId: string, tokenScopes: Set<string>, scopeMapping: Record<string, string[]>): boolean {
...
async function getActiveProviderScopes(db: ReturnType<typeof drizzle<typeof schemaAndRelations>>, orgId: string): Promise<Set<string>> {
...
export async function capabilitiesRoutes(app: FastifyTypeBox) {
  ...
  app.get(
    ...
    async (
      ...
      return reply.status(200).send(capabilities.map((provider) => {
        return {
          ...provider,
          capabilities: provider.capabilities.map((capability) => {
            return { ...capability, available: hasPermissions(capability.id, activeScopes, provider.scopeMapping) };
          }),
        }
      }));
    },
  ...
  app.get(`/capabilities/:providerId/:capabilityId`, {
    schema: CapabilityEndpointSchema,
  }, async (request: FastifyRequestTypeBox<typeof CapabilityEndpointSchema>, reply: FastifyReplyTypeBox<typeof CapabilityEndpointSchema>) => {
    ...
    return reply.status(200).send({
      ...
      capability: {
        ...capability.meta,
        available: hasPermissions(capability.meta.id, activeScopes, provider!.scopeMapping),
      },
    });
  });
