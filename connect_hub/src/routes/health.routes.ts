...
export async function healthRoutes(app: FastifyTypeBox) {
  ...
  app.get('/health', {
    ...
    request: FastifyRequestTypeBox<{ response: { 200: typeof HealthResponseSchema } }>, 
    reply: FastifyReplyTypeBox<{ response: { 200: typeof HealthResponseSchema } }>
  ) => {
    ...
    try {
      await app.databaseService.healthCheck();
      dbStatus = 'connected';
    } catch (error) {
