...
test('createMultiAgentSystem should decompose job and create agents', async () => {
  ...
  expect(result.jobDescription).toBe('Please schedule a team meeting and send confirmations');
...
test('createMultiAgentSystem should handle complex job descriptions', async () => {
  ...
  vi.mocked(mockLLMProvider.createCompletion)
    ...
    .mockResolvedValueOnce({
      content: JSON.stringify({
        'Travel Planner': ['Research destination options', 'Book flights and hotels'],
  ...
  const travelPlannerAgent = result.agents.find(a => a.name === 'Travel Planner');
