...
import { ErrorSchema } from "../models/index.js";
import {
  GetPromptsEndpoint,
  GetPromptEndpoint,
  PromptTemplate,
...
export default async function promptRoutes(fastify: FastifyTypeBox) {
  ...
  // Set active version
  fastify.put(
    "/v1/prompts/:promptId/active/:version",
    {
  ...
  fastify.get(
    "/v1/prompts/:promptId/compare/:version1/:version2",
    {
    ...
    async (
      ...
      try {
        const comparison = promptRegistry.compareVersions(
          promptId,
          version1,
          version2,
        );
      ...
      try {
        ...
        for (const result of results) {
          ...
          if (metrics) {
            ...
            const score =
              (metrics.successRate || 0) * 0.6 +
              (metrics.qualityScore || 0) * 0.04;
            if (score > bestScore) {
              bestScore = score;
