...
import { GetApprovalRequestsEndpoint, GetApprovalRequestEndpoint, CreateApprovalRequestEndpoint, UpdateApprovalRequestEndpoint, ApprovalRequestSchema } from "../models/approvals";
...
export const approvalRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  }, async (request: FastifyRequestTypeBox<typeof GetApprovalRequestsEndpoint>, reply) => {
    const filters = []
  ...
  }, async (request: FastifyRequestTypeBox<typeof GetApprovalRequestEndpoint>, reply) => {
  ...
  }, async (request: FastifyRequestTypeBox<typeof CreateApprovalRequestEndpoint>, reply) => {
    const {createdAt, updatedAt, ...toInsert}  = request.body
    const approval = await fastify.db.insert(approvalRequests).values(toInsert).returning();
    return reply.status(200).send(approval[0]);
  });

  fastify.put("/approvals/:id", {
    schema: UpdateApprovalRequestEndpoint,
  }, async (request: FastifyRequestTypeBox<typeof UpdateApprovalRequestEndpoint>, reply) => {
    const id = request.params.id
    const { ...toUpdate} = request.body
    const approvals = await fastify.db.update(approvalRequests).set(toUpdate).where(eq(approvalRequests.id, id)).returning();
