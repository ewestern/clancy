...
import { promptRegistry, type PromptExecution } from './prompts/promptRegistry.js';
...
export class MultiAgentGraphCreator {
  ...
  async decomposeJobDescription(jobDescription: string, promptVersion?: string): Promise<TaskDecomposition[]> {
    const executionId = generateUuid();
    const startTime = Date.now();
    
    try {
      // Get prompt from registry
      const promptTemplate = promptRegistry.getPrompt('job-decomposition', promptVersion);
      if (!promptTemplate) {
      ...
      const response = await this.llmProvider.createCompletion({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert at decomposing complex jobs into manageable tasks for AI agents. Always return valid JSON."
          },
          {
            role: "user",
            content: prompt
  ...
  private async identifyAgentGroups(tasks: TaskDecomposition[], promptVersion?: string): Promise<Record<string, TaskDecomposition[]>> {
    const executionId = generateUuid();
    const startTime = Date.now();

    try {
      // Get prompt from registry
      const promptTemplate = promptRegistry.getPrompt('agent-grouping', promptVersion);
      if (!promptTemplate) {
      ...
      const response = await this.llmProvider.createCompletion({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert at organizing tasks into cohesive agent responsibilities. Always return valid JSON."
          },
          {
            role: "user",
            content: prompt
      ...
      for (const [agentName, taskDescriptions] of Object.entries(agentGroups)) {
        result[agentName] = (taskDescriptions as string[]).map(desc =>
          tasks.find(t => t.taskDescription === desc)
  ...
  private evaluateGroupingQuality(groups: Record<string, TaskDecomposition[]>, allTasks: TaskDecomposition[]): number {
    let score = 5; // Base score
    ...
    const variance = tasksPerGroup.reduce((sum, count) => sum + Math.pow(count - avgTasksPerGroup, 2), 0) / groupCount;
    
    if (variance < 2) { // Well-distributed
      score += 1;
    }
  ...
  private async createAgentFromTasks(
    ...
    const allCapabilities = [...new Set(tasks.flatMap(t => t.requiredCapabilities))];
  ...
  private determineOverallComplexity(tasks: TaskDecomposition[]): string {
    ...
    const avgComplexity = tasks.reduce((sum, task) => 
      sum + complexityScores[task.estimatedComplexity], 0
    ) / tasks.length;
  ...
  async testPromptVersion(
    ...
    for (let i = 0; i < iterations; i++) {
      if (promptId === 'job-decomposition') {
        const result = await this.decomposeJobDescription(jobDescription, version);
        results.push(result);
      }
